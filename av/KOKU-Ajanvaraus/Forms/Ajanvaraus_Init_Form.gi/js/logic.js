...
function commitCustomAutoRowSession(matrix, cache) {
    var nodes, xmlStr;
    matrixElement = AjanvarausForm.getJSXByName(matrix.valueOf());
...
function mapSelectedRecipientsToMatrix() {
    ...
    while(childIterator.hasNext()) {
        ...
        // parent's uid
        uid = childNode.getAttribute("recipientsUid");
...
function modifyThisSection(panelName) {
    var slot, modPane, attributes, section, id;
    
    if (panelName.search("panel") !== -1) {
        id = panelName.slice(5, panelName.length);
    } else {
        id = panelName;
    }

    checkIfModifyOpen();
    slot = AjanvarausForm.getJSXByName(panelName);
    modPane = slot.getDescendantOfName("modPane");
    attributes = getSlotAttributes(id);
    setValuesForModify(panelName, 1);
...
function saveValues(panelName) {
    ...
    if(infotext !== "") {
        AjanvarausForm.getJSXByName(panelName).getDescendantOfName("infotext").setValue(infotext);
        AjanvarausForm.getJSXByName(panelName).getDescendantOfName("tooltipImg").setDisplay("block", true);
    } else {
        AjanvarausForm.getJSXByName(panelName).getDescendantOfName("tooltipImg").setDisplay("none", true);
    }

    AjanvarausForm.getJSXByName(panelName).getDescendantOfName("entry").setText(appDateEntry + ", klo: " + startTime + " - " + endTime + ", " + locat, true);
}
 ...
 */
function removeThisSection(panelName) {
    var panel, id;
    ...
    panel = AjanvarausForm.getJSXByName("calendarEntryBlock").getChild(panelName);
    ...
    AjanvarausForm.getJSXByName("calendarEntryBlock").removeChild(AjanvarausForm.getJSXByName(panelName));
...
function mapRecipients(objXML) {
    ...
    for( i = 0; i < recipients.length; i = i + 1) {
        ...
        node.setAttribute("targetPerson", recipients[i][1]);
...
function disableForm() {
    ...
    AjanvarausForm.getJSXByName("User_Role").setEnabled(false, true);
    AjanvarausForm.getJSXByName("User_SenderRealName").setReadonly(true);
    AjanvarausForm.getJSXByName("User_SenderRealName").repaint();
    AjanvarausForm.getJSXByName("Header_Text").setReadonly(true);
    AjanvarausForm.getJSXByName("Header_Text").repaint();
    AjanvarausForm.getJSXByName("Header_Description").setReadonly(true);
    AjanvarausForm.getJSXByName("Header_Description").repaint();
    AjanvarausForm.getJSXByName("tapaamistenLisaaminen").setDisplay("none", true);
    buttons = AjanvarausForm.getJSXByName("tapaamiset").getDescendantsOfType("jsx3.gui.Button");

    for (var i = 0; i < buttons.length; i++) {
        buttons[i].setDisplay("none", true);
    }
    
    setButtonDisplay("IntalioInternal_StartButton", "none");
}

function setButtonDisplay(label, display) {
    $('span[label|="' + label + '"]').css('display', display);
}

function getTaskSubscribe() {
    Intalio.Internal.Utilities.SERVER.subscribe(Intalio.Internal.Utilities.GET_TASK_SUCCESS, Preload);
};
 ...
 */
function Preload() {
    var username, formData, id, uid, uidData, userRealName, currentUserUid;
    ...
    if(gup("FormID")) {
        ...
        try {
            ...
            if(formData !== null) {
                ...
                senderUid = formData.selectSingleNode("//uid", "xmlns:ns2='http://soa.common.koku.arcusys.fi/'").getValue();
                if(senderUid != uid) {
                    disableForm();
                }
            }
