	...
	<xsl:template match="fe:Ajanvaraus_Form">
		<xsl:copy>
			<xsl:element name="appointment">
				...
				<!--
					<xsl:for-each select="//fe:Recipients"> <xsl:if test =
					"Recipients_TargetPerson/text() != $tp" > <xsl:element
					name="receipients"> <xsl:element name="receipients"> <xsl:value-of
					select="fe:Recipients_Recipient/text()"/> </xsl:element>

					<xsl:element name="targetPerson"> <xsl:value-of
					select="fe:Recipients_TargetPerson/text()"/> </xsl:element>
